 ...
 * the same thing.
 */
/* `number-from` is where the movements count from: 1, the default, or 0.
 * A talk whose first movement is an opening rather than a beat can number
 * it 0, so that beat three's slides are 3.x and the number on a section
 * slide is the number in the slide's corner. */
const IDENTITY = ['name', 'title', 'number-from'];
...
function identity(src) {
  ...
  let from = 1;
  if (front['number-from'] !== undefined) {
    if (!['0', '1'].includes(front['number-from'])) {
      throw new Error(`deck.md: number-from is ${front['number-from']}; it is 0 or 1, ` +
        'the number of the first movement.');
    }
    from = Number(front['number-from']);
  }

  return { name: front.name, title: front.title || front.name, key: `sipario:${slug(front.name)}`, from };
}
...
/* Keyed by path and checked against the file's mtime on every read, so
 * an edited figure is read again and an unchanged one is not. The cache
 * used to be keyed by path alone and never emptied, which meant a drawing
 * edited mid-rehearsal never reached the deck: the save reloaded the page,
 * the render came back for the figure, and the cache handed it the old
 * one for as long as the server ran. */
const figureCache = new Map();

function readFigure(name, root) {
  const file = path.join(root, `${name}.svg`);
  const mtime = fs.statSync(file).mtimeMs;
  const hit = figureCache.get(file);
  if (!hit || hit.mtime !== mtime) {
    figureCache.set(file, { mtime, svg: fs.readFileSync(file, 'utf8').trim() });
  }
  return figureCache.get(file).svg;
}
...
function checkDeclaredIds(groups, from = 1) {
  const wrong = [];
  groups.forEach((g, gi) => {
    g.slides.forEach((sl) => {
      const n = sl.meta.slide;
      if (!sl.meta.id) wrong.push(`slide ${n} declares no id`);
      else if (!sl.meta.id.startsWith(`${gi + from}-`)) {
        wrong.push(`slide ${n} has id "${sl.meta.id}" but sits in section ${gi + from} (${g.name})`);
      }
    });
  });
  if (wrong.length) {
...
/* A script is paragraphs, and a paragraph is a list when its first line
   opens with a marker. Read once here and written out twice, as markup
   for the notes window and as text for the notes a PowerPoint export
   carries, so the two can never read one script differently. */
function noteBlocks(note) {
  ...
  return note.trim().split(/\n\s*\n/).map((block) => {
    ...
    for (const [marker, tag] of [[BULLET, 'ul'], [NUMBER, 'ol']]) {
      if (marker.test(lines[0])) return { tag, items: noteItems(lines, marker) };
    }
    const line = lines.join(' ');
    return { tag: 'p', cue: line.startsWith('[') && line.endsWith(']'), text: line };
  });
}

function notesHtml(note) {
  if (!note) return '';
  return noteBlocks(note).map((b) => {
    if (!b) return '';
    if (b.items) return `<${b.tag}>${b.items.map((t) => `<li>${inline(t)}</li>`).join('')}</${b.tag}>`;
    return `<p${b.cue ? ' class="cue"' : ''}>${inline(b.text)}</p>`;
  }).join('\n          ');
}

/* The same script as text, for a notes pane: an item per line with its
   marker, a blank line between paragraphs, and the emphasis markers
   dropped since a notes pane has nowhere to put them. A cue keeps its
   brackets; on a lectern those are what marks it as one. */
function notesText(note) {
  const plain = (t) => t
    .replace(/\*\*(?=\S)([\s\S]*?\S)\*\*/g, '$1')
    .replace(/\*(?=\S)([^*]*?\S)\*/g, '$1');
  return noteBlocks(note).filter(Boolean).map((b) => {
    if (b.items) {
      return b.items.map((t, i) => (b.tag === 'ol' ? `${i + 1}. ` : '\u2022 ') + plain(t)).join('\n');
    }
    return plain(b.text);
  }).join('\n\n');
}
...
function numberOf(gi, si, from = 1) {
  return `${gi + from}.${si + 1}`;
}

function checkNumbers(groups, from = 1) {
  const wrong = [];
  groups.forEach((g, gi) => {
    g.slides.forEach((sl, si) => {
      const want = numberOf(gi, si, from);
...
function render(src, opts = {}) {
  ...
  checkNumbers(groups, deck.from);
  checkDeclaredIds(groups, deck.from);
  checkAlign(slides);
  checkRenamed(slides);
  checkNotes(slides);
  checkSteps(slides);
  ...
  const blocksOut = groups.map((g, gi) => {
    ...
    const sec = gi + deck.from;
    const stacks = g.slides.map((sl, si) => {
      const num = numberOf(gi, si, deck.from);
...
module.exports = { render, parse, identity, checkNumbers, grouped, checkIds, checkDeclaredIds,
                   ...
                   stepId, numberOf, notesText, slug };
