 ...
 *
 * Node's own http module and nothing else. The server answers eight
...
const { deckPage, presenterPage, printPage, errorPage } = require('./pages.js');
...
function serve(dir, { port = Number(process.env.PORT) || 9999, log = console } = {}) {
  ...
  function handle(req, res) {
    ...
    if (pathname === '/') {
      ...
      return page(res, () => deckPage(renderedAt().r),
        (err) => send(res, 500, 'text/html; charset=utf-8', errorPage(err)));
    }
    if (pathname === '/presenter') {
      return page(res, () => { const at = renderedAt(searchParams.get('v')); return presenterPage(at.r, port, at.version); },
        (err) => send(res, 500, 'text/plain; charset=utf-8', String(err.stack || err.message)));
    }
    if (pathname === '/print') {
      return page(res, () => printPage(renderedAt().r),
        (err) => send(res, 500, 'text/html; charset=utf-8', errorPage(err)));
    }
  ...
  server.on('close', () => { clearTimeout(pending); for (const w of watchers) w.close(); });
