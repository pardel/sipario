...
## 1.1 A theatre bill in black and red
id: 1-a-slide-of-every-template
template: title
kicker: STYLISH
subtitle: Black and curtain-red on paper, a swelled rule, and rows set as a cast list
author: Mario Rossi

```notes
The same deck as starters/minimal, slide for slide, in a different
dress: sipario is the curtain, so this one is set like the bill outside
an Italian theatre. The suite renders it too, so the look cannot rot.
```
...
The curtain comes down between the acts.
...
```notes
T is for Templates. A section slide is the act break, and this theme puts it
on the curtain itself: velvet ground, every ink turned to paper, the letter
at the size of the stage.
```
...
Black and red on paper, set in Newsreader.
...
The size of a line says how much it matters.
...
- **S** Sipario — the curtain, and the name of the engine
- **T** Type — one family in two voices, and the size says the rest
- **Y** Yours — copy the folder, and the look travels with the talk
...
id: 4-take-it
template: close
colophon: starters/stylish · docs/AUTHORING.md · docs/TEMPLATES.md
